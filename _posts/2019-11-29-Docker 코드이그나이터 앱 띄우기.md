...
추가로 vurtual_host를 어떻게 구현해야할지..잘 안되고있습니다.
실행할때 옵션으로 포트 구분해서 개발 서버 사용중 (port와 documentroot변경)
docker run -p 81:80 --rm \
  -v $(pwd):/var/www/html \
  -e APACHE_DOCUMENT_ROOT=/var/www/html/www \
  hojindev/codeigniter_hmvc:latest
  
  vitual host 잘 안되네...일단 포트로 나눠서 도커 여러개에 나눠서 띄움 
  -> 로그메시지가 컨테이너별로(터미널) 출력되어 개발 서버로는 나쁘지 않음
